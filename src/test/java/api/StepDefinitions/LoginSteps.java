...
public class LoginSteps {
	...
	public void admin_creates_request_with_credentials_with(String scenario) throws IOException {
	    ...
	    for (Map<String, String> row : excelData) {
	        ...
	        // Compare the scenario from the feature file with the Excel data scenario
	        String excelScenario = currentRow.get("ScenarioName");

	        // If the scenario in Excel matches the one from the feature file, proceed with building the request
	        if (excelScenario.equalsIgnoreCase(scenario)) {
	            ...
	            break; // Exit loop after processing the matching scenario
	        }
	    }
	 // If no matching scenario is found, throw an exception
        if (!scenarioFound) {
	...
	public void admin_calls_post_method_with_endpoint() {
		 // Make sure requestSpec is not null before sending the request
        if (requestSpec == null) {
	...
	public void admin_receives_status_code_with_status_text() {
	   ...
	   // String actualStatusMessage = response.jsonPath().getString("message");
